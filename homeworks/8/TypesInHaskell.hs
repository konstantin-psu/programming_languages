...
{- To work with this file, you will need either the Glasgow Haskell 
   interpreter (ghci) or the Hugs interpreter (hugs) -- 
   either should work fine.  They ar installed on the 
   linux lab machines. To download to your own machine, visit
   www.haskell.org/ghc or www.haskell.org/hugs.
...
first'n'third :: [Int] -> (Int,Int)
first'n'third li = (head li, head(tail(tail(li))))
...
(c) Write and test a function with type
...
eqpair (second,second)  -- static error: the type of second ([a] -> a) 
  ...
  but we won't see those details today.
-}
...
{- The built-in String type is actually defined (in the Prelude) as 
   ...
   Exercise: write and test a function of type String -> Char 
   that returns the second character of a String.                -}

secondc :: String -> Char
secondc st = second(st)
...
{- This definition does two things:
   ...
   They behave very much like constructors in Java, except that they
   do not execute arbitrary code: they _just_ construct a value from
   its constituent fields. For example, we can create the small tree
...
sizeTree t =
    case t of
        Leaf v -> 1
        Node v tr1 tr2 -> 1 + sizeTree(tr1) + sizeTree(tr2)
...
{- Challenge Exercise: Write and test a function:
...
myzip a b = 
    case (a,b) of
        (ah:at,bh:bt) -> (ah,bh): myzip at bt
        ([],[]) -> []
        (_,_) -> []
-- TODO FIX ME       ([],bh:bt) -> ([]::a,bh): myzip () bt
...
{- Try evaluating the test tree.
   ...
   Simple Exercise: Add a MulE node (for multiplication) to
   the Exp data type and the evaluator, and test it.

-}           

mt :: Exp
mt  = MulE (NumE 4) (NumE 5)
...
{- Try out these expressions:
  ...
  Exercise: Define single-constructor data types for
